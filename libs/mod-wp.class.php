...
//first,create pdo to access the server since the db may not exist
            $db = new PDO(
                    ...
                    , $this->SITE_CONFIG[ 'wp_config' ][ 'DB_USER' ]
                    , $this->SITE_CONFIG[ 'wp_config' ][ 'DB_PASSWORD' ]
            );



            //attempt to use the db
            $success = $db->query( "USE `" . $this->SITE_CONFIG[ 'wp_config' ][ 'DB_NAME' ] . "`" );

            if ( !$success ) { // if not successful, it doesn't exist, so create it
                $success = $db->query( "CREATE DATABASE IF NOT EXISTS " . "`" . $this->SITE_CONFIG[ 'wp_config' ][ 'DB_NAME' ] . "`" );
            ...
            $db = new PDO(
                    'mysql:host=localhost;' . "dbname=" . $this->SITE_CONFIG[ 'wp_config' ][ 'DB_NAME' ]
                    , $this->SITE_CONFIG[ 'wp_config' ][ 'DB_USER' ]
                    , $this->SITE_CONFIG[ 'wp_config' ][ 'DB_PASSWORD' ]
            );

            //use the db
            $db->query( "USE `" . $this->SITE_CONFIG[ 'wp_config' ][ 'DB_NAME' ] . "`" );
...
//returns false if no results, otherwise returns the first table name.
    ...
    private function _deleteDB() {
        ...
        try {
            $result = true;
            $success = $db->query( "DROP DATABASE " . $this->SITE_CONFIG[ 'wp_config' ][ 'DB_NAME' ] );
            ...
            if ( $exc->getCode() === 1044 ) { //if you don't have permissions , show the message associated with the exception.
                ...
                $this->_ERROR_MESSAGES[] = gettext( "Error dropping the existing database " . $this->SITE_CONFIG[ 'wp_config' ][ 'DB_NAME' ] . "  . Drop the database manually and try again" );
            }
            if ( $this->SITE_CONFIG[ 'debug-show-exceptions' ] ) {
...
//reinstall set to true will force file and database overwrites
        ...
        //set admin passwords
        $this->SITE_CONFIG[ 'wp_users' ][ 'user_pass' ] = $this->getPassword();
