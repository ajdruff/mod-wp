...
class modwp_install {
    ...
    public $CONFIG_DEFAULTS = null;
    public $SITE_CONFIG = null; //contains all the variables in config-site.json
    public $PROFILE_CONFIG = null; //contains all the variables in config-profile.json 
    ...
    private function _config() {
        ...
        if ( $this->CONFIG[ 'general' ][ 'max_execution_time' ] != 0 && is_numeric( $this->CONFIG[ 'general' ][ 'max_execution_time' ] ) ) {
            ini_set( 'max_execution_time', $this->CONFIG[ 'general' ][ 'max_execution_time' ] );
        }
        ...
        $this->CONFIG_PROPERTIES_FILE = $this->INSTALLER_DIRECTORY . '/libs/mod-wp.config.json';
    ...
    private function _init() {
        ...
        if ( !$this->phpLib()->isCommandLine() ) { //set error handler when not in command line. setting an error handler within command line will cause loss of output after WP Library is included.
...
// Build the absolute path of the configured director. If not provided, use the parent directory of this file
        $this->WP_DIRECTORY = !empty( $this->CONFIG[ 'general' ][ 'wp_directory' ] ) ? dirname( $this->INSTALLER_DIRECTORY ) . '/' . $this->CONFIG[ 'general' ][ 'wp_directory' ] : dirname( $this->INSTALLER_DIRECTORY );
...
//   $this->_WARNING_MESSAGES[] = '<strong>' . _( 'Security Warning' ) . '</strong>: To keep your site secure: ' . _( 'Delete this     directory: <span class="well">' . $this->INSTALLER_DIRECTORY . '</span>' );
        ...
        $tags[ '{INSTALLER_DIRECTORY}' ] = $this->INSTALLER_DIRECTORY . '/config-site.json';
...
// We add the extras constant
                ...
                case 'DB_HOST' :
                    ...
                    break;
                case 'SCRIPT_DEBUG':
                    if ( !empty( $this->CONFIG[ 'general' ][ 'script_debug' ] ) ) {
                        $line .= "\r\n\n " . "/** SCRIPT_DEBUG = true for non-minified javascript */" . "\r\n\n\n";
                        $line .= "define('SCRIPT_DEBUG', " . sanit( $this->CONFIG[ 'general' ][ 'script_debug' ] ) . ");\r\n";
                    }
                    break;
                case 'AUTH_KEY' :
        ...
        switch ( $environment ) {
            ...
            case "dev":
                if ( $this->CONFIG[ 'general' ][ 'move_wpconfig' ] ) {
...
//make config directory
    ...
    private function _wpCreateWPConfigHome() {
        ...
        $is_web_root = trim( $this->CONFIG[ 'general' ][ 'wp_directory' ] === '' );
...
// WordPress Downloaded but in danger of being overwritten?
        if ( $files_overwrite && file_exists( $this->WP_DIRECTORY . '/wp-includes' ) && $this->CONFIG[ 'general' ][ 'reinstall' ] !== true ) {




            $this->_ERROR_MESSAGES[] = gettext( "You attempted to download and overwrite an existing WordPress Installation. To fix this issue, set 'general:wp_directory' to  a different install directory, delete the WordPress files (excluding the 'mod-wp' directory) in the existing directory " . $this->WP_DIRECTORY . ", or set 'install_switches:wpDownload' to false to skip download. You may also set 'general:reinstall' to true to overwrite the existing WordPress installation." );
            $this->_displayMessages();
            die();
        }
// WordPress Available?
        if ( $files_available && !file_exists( $this->WP_DIRECTORY . '/wp-includes' ) ) {
            $this->_ERROR_MESSAGES[] = gettext( "WordPress has not been downloaded yet into the configured directory set by 'general:wp_directory' (" . $this->CONFIG[ 'general' ][ 'wp_directory' ] . ") Please download WordPress by editing `config-site.json` and setting 'install_switches:wpDownload' to false or set 'general:wp_directory'  to a directory that contains the downloaded WordPress files." );
            $this->_displayMessages();
            die();
        }
...
//if not empty and reinstall is true, delete the database.
            ...
            if ( !$this->_isDBEmpty() && $this->CONFIG[ 'general' ][ 'reinstall' ] ) {
                $this->_deleteDB();
                ...
                $this->_ERROR_MESSAGES[] = gettext( 'WordPress database `' ) . $this->CONFIG[ 'wp_config' ][ 'DB_NAME' ] . gettext( "` is not empty. To fix this issue, set 'install_switches:wpInstallCore' to false, empty the existing database `" ) . $this->CONFIG[ 'wp_config' ][ 'DB_NAME' ] . gettext( "`, or  set 'wp_options:DB_NAME' to a different empty database to use for installation.You may also set 'general:reinstall' to true to overwrite the existing WordPress database." );
                $this->_displayMessages();
                die();
            }
        }
        ...
        if ( $cmd_requirements ) {
            if ( $this->CONFIG[ 'general' ][ 'HTTP_HOST' ] === 'example.com' || trim( $this->CONFIG[ 'general' ][ 'HTTP_HOST' ] ) === '' ) {

                $this->_ERROR_MESSAGES[] = gettext( "You must set 'general:HTTP_HOST' to a valid domain before installing or your site will not be accessible once installed." );
                $this->_displayMessages();
                die();
            }
        }
    ...
    public function wpDownload() {
        ...
        if ( !$this->CONFIG[ 'install_switches' ][ 'wpDownload' ] ) {
...
//check if file object exists first, if it does, delete the destination file or directory. This is normal behavior for rename but it doesn't always work on windows systems so we need to do an explicit delete. we use the phpLib->rmDir because it deletes both files and directories.
        ...
        if ( $this->CONFIG[ 'general' ][ 'reinstall' ] == true ) {
    ...
    public function _wpRemoveFileCruft() {
        if ( !$this->CONFIG[ 'general' ][ 'remove_file_cruft' ] ) {
            return;
        }
    ...
    public function wpConfig() {
        ...
        if ( !$this->CONFIG[ 'install_switches' ][ 'wpConfig' ] ) {
...
//check that files are available to configure
        ...
        if ( $this->CONFIG[ 'general' ][ 'slide_rule' ] === true ) {
...
//Now use the key to find the right action and execute it.
            ...
            try {
                ...
                if ( $this->_RETRY < $this->_RETRY_MAX ) {
                    ...
                    if ( $this->CONFIG[ 'general' ][ 'show_exceptions' ] ) {
    ...
    public function wpInstallCore() {
        ...
        if ( !$this->CONFIG[ 'install_switches' ][ 'wpInstallCore' ] ) {
...
//set WP_SITEURL before install otherwise (especially in command line mode) wp_install will 'guess' and incorrectly set it, resulting in a sockets error
        ...
        wp_install(
                ...
                $this->CONFIG[ 'general' ][ 'allow_search_engines' ], //$public Whether site is public
                '', //$deprecated
...
//show password
    ...
    private function _wpAddInstallDropin() {
        ...
        if ( $this->CONFIG[ 'general' ][ 'install_dropin' ] ) {
...
//return if user doesn't want to add
        if ( !$this->CONFIG[ 'general' ][ 'add_custom_content' ] ) {
            return;
        }

//return if posts don't exist
        if ( !isset( $this->CONFIG[ 'content' ][ 'posts' ] ) || !is_array( $this->CONFIG[ 'content' ][ 'posts' ] ) ) {
            return;
        }
        ...
        foreach ( $this->CONFIG[ 'content' ][ 'posts' ] as $post ) {
...
// Let's create the page
    ...
    private function _wpAddCustomThemes() {
        ...
        if ( !$this->CONFIG[ 'general' ][ 'add_custom_themes' ] ) {
...
// Unzip the archive in the plugin folder
    ...
    public function wpInstallThemes() {
        ...
        if ( !$this->CONFIG[ 'install_switches' ][ 'wpInstallThemes' ] ) {
    ...
    public function wpInstallPlugins() {
        ...
        if ( !$this->CONFIG[ 'install_switches' ][ 'wpInstallPlugins' ] ) {
...
//include WordPress Libraries
        ...
        if ( isset( $this->CONFIG[ 'general' ][ 'plugins' ] ) && !empty( $this->CONFIG[ 'general' ][ 'plugins' ] ) ) {


            $plugins = $this->CONFIG[ 'general' ][ 'plugins' ];
...
// We unzip it
        ...
        if ( $this->CONFIG[ 'general' ][ 'add_custom_plugins' ] == 1 && file_exists( $this->PROFILE_DIRECTORY . '/plugins' ) ) {
...
// We unzip the archive in the plugin folder
        ...
        if ( $this->CONFIG[ 'general' ][ 'activate_plugins' ] == 1 ) {
...
//first,create pdo to access the server since the db may not exist
            ...
            if ( $exc->getCode() === 1044 ) { //if you don't have permissions , show the message associated with the exception.
                ...
                $this->_ERROR_MESSAGES[] = gettext( "Error Establishing Database Connection. Check `config-site.json` and verify that the database connection settings are correct." );
            }
            if ( $this->CONFIG[ 'general' ][ 'show_exceptions' ] ) {
...
//if not resetting password, or if we are installing core, return
        if ( !($this->CONFIG[ 'install_switches' ][ 'wpResetPassword' ] && !$this->CONFIG[ 'install_switches' ][ 'wpInstallCore' ]) ) {
...
//   $this->_WARNING_MESSAGES[] = '<strong>' . _( 'Security Warning' ) . '</strong>: To keep your site secure: ' . _( 'Delete this     directory: <span class="well">' . $this->INSTALLER_DIRECTORY . '</span>' );
        ...
        $tags[ '{RESET_INSTRUCTIONS}' ] = gettext( "This password will not be emailed to you. If you need to reset it, re-run the setup script using 'install_switches:wpResetPassword' set to true and  'install_switches:wpInstallCore' set to false or use WordPress's 'lost password' feature." );
...
// We update the options with the right siteurl et homeurl value
        ...
        $url = $protocol . '://' . $_SERVER[ 'HTTP_HOST' ] . '/' . $this->CONFIG[ 'general' ][ 'wp_directory' ];
    ...
    private function _wpRemoveDefaultContent() {
        ...
        if ( $this->CONFIG[ 'general' ][ 'remove_default_content' ] == 1 ) {
    ...
    public function _wpSetMediaOptions() {
        ...
        if ( !$this->CONFIG[ 'general' ][ 'set_media_options' ] ) {
            return;
        }
        /* -------------------------- */
    ...
    private function _wpRemoveDefaultPlugins() {
        ...
        if ( !$this->CONFIG[ 'general' ][ 'remove_default_plugins' ] ) {
            return;
        }
...
//if a file, delete it unless its wp-plugins/index.php
     ...
     *
     * Strips out the comments added to a json array
     *
     * @param none
     * @return void
     */
    public function removeJsonComments( $json ) {
        if ( isset( $config[ 'wp_config' ] ) ) {
            $json[ 'wp_config' ] = array_filter( $json[ 'wp_config' ] );
        }
        if ( isset( $config[ 'wp_options' ] ) ) {
            $json[ 'wp_options' ] = array_filter( $json[ 'wp_options' ] );
        }
        if ( isset( $config[ 'wp_users' ] ) ) {
            $json[ 'wp_users' ] = array_filter( $json[ 'wp_users' ] );
        }
        if ( isset( $config[ 'general' ] ) ) {
            $json[ 'general' ] = array_filter( $json[ 'general' ] );
        }
        unset( $json[ 'comment' ] );
        unset( $json[ 'general' ][ 'comment' ] );
        unset( $json[ 'wp_config' ][ 'comment' ] );
        unset( $json[ 'wp_options' ][ 'comment' ] );
        unset( $json[ 'wp_users' ][ 'comment' ] );
        unset( $json[ '#' ] );
        unset( $json[ 'general' ][ '#' ] );
        unset( $json[ 'wp_config' ][ '#' ] );
        unset( $json[ 'wp_options' ][ '#' ] );
        unset( $json[ 'wp_users' ][ '#' ] );
    ...
    public function sandbox() {
        return;
        $config=$this->getSafeConfig( $this->CONFIG );
...
return;
        ...
        return;
        $this->createConfigSample( 'profile' );
        $this->createConfigSample( 'site' );
        die();
        return;
        $this->_setConfigProps();


        $this->_setConfigDefaults();
        echo '<pre>', print_r( $this->CONFIG_PROPS, true ), '</pre>';

        echo '<pre>', print_r( $this->CONFIG_DEFAULTS, true ), '</pre>';
...
// Note : The theme is automatically activated if the user asked to remove the default theme
                if ( $this->CONFIG[ 'activate_theme' ] == 1 || $this->CONFIG[ 'general' ][ 'remove_default_themes' ] == 1 ) {
                    switch_theme( $theme_name, $theme_name );
                }

// Let's remove the Twenty family
                if ( $this->CONFIG[ 'general' ][ 'remove_default_themes' ] == 1 ) {
...
// We delete the _MACOSX folder (bug with a Mac)
    ...
    public function _wpInstallThemes() {
        ...
        if ( $this->_wpThemeExists() && ($this->CONFIG[ 'general' ][ 'remove_default_themes' ]) ) {
...
//if it made it this far, it didn't find it.
    ...
    public function _setConfigProps() {
        ...
        //read the file
        $config_props = file_get_contents( $this->CONFIG_PROPERTIES_FILE );

        //parse json into an array
        $config_props = array_filter( json_decode( $config_props, true ) );

//remove json comments
        $this->CONFIG_PROPS = $this->removeJsonComments( $config_props );
    }
    ...
    public function _setConfig() {
        ...
        $this->_setConfigProps(); //$this->CONFIG_PROPS       
        $this->_setConfigDefaults(); //$this->CONFIG_DEFAULTS
        ...
        $this->CONFIG = $this->phpLib()->arrayMerge( $this->CONFIG_DEFAULTS, $this->PROFILE_CONFIG );
        ...
        if ( $this->phpLib()->isCommandLine() ) {
            $_SERVER[ 'HTTP_HOST' ] = $this->CONFIG[ 'general' ][ 'HTTP_HOST' ];
        }
    }
     ...
     * Set Config Defaults
     *
     * Builds a default array from the properties file. 
     * 
     * @param none
     * @return void
     */
    private function _setConfigDefaults() {
        ...
        $config = array();
        foreach ( $this->CONFIG_PROPS as $all_config_props_key => $config ) {

            $config_key = null;
            $props = array();
            foreach ( $config as $config_key => $props ) {
                $props_key = null;
                $props_value = array();
                ...
                if ( is_array( $props ) ) {
                    ...
                    foreach ( $props as $props_key => $props_value ) {
                        if ( $props_key === 'default' ) {
                            $this->CONFIG_DEFAULTS[ $all_config_props_key ][ $config_key ] = $props_value;
                        }
                    }
                }
            }
        ...
        $this->CONFIG_DEFAULTS = $this->phpLib()->convertToBool( $this->CONFIG_DEFAULTS );
    }
     ...
     * @return void
     */
    private function _setConfigDefaults_old() {


        include($this->CONFIG_PROPERTIES_FILE); //include defaults in case the selected config file doesnt have all the required settings




        $this->CONFIG_DEFAULTS = $config; //$config is defined in config file
    ...
    private function _setSiteConfig() {
        ...
        //parse json into an array
        $config = array_filter( json_decode( $config, true ) );


        //remove comments;
        $config = $this->removeJsonComments( $config );



        $this->SITE_CONFIG = $config; //$config is defined in the site configuration file config-site.json . 
...
//do the same with $_POST since you'll be posting the form  on submit      
     ...
     * @return void
     */
    private function _setSiteConfig_old() {
        if ( !is_null( $this->SITE_CONFIG ) ) {
            return;
        }
...
//do the same with $_POST since you'll be posting the form  on submit      
    ...
    private function _setProfileConfig() {
        ...
        $this->PROFILE_DIRECTORY = $this->PROFILES_DIRECTORY . '/' . $this->SITE_CONFIG[ 'general' ][ 'profile' ];
        $this->PROFILE_CONFIG_FILE = $this->PROFILE_DIRECTORY . '/config-profile.json';


        //read the file
        $config = file_get_contents( $this->PROFILE_CONFIG_FILE );
        ...
        $this->PROFILE_CONFIG = $config; //$config is defined in the profile configuration file config-profile.json . 
        //replace any variables with those that are contained in the GET request (for initial page build/page refresh)
...
//do the same with $_POST since you'll be posting the form  on submit      
     ...
     * @return void
     */
    private function _setProfileConfig_old() {
        if ( !is_null( $this->PROFILE_CONFIG ) ) {
            return;
        }
...
//            
     ...
     *
     * Scrubs the configuration array of any sensitive security data (any setting mod-wp.config.json whose 'view_web' property is set to false)
     *
    ...
    public function getSafeConfig( $config ) {
        ...
        foreach ( $this->CONFIG_PROPS as $section_name=>$settings ) {
               foreach ( $settings as $setting_name=>$setting_props ) {
             //      echo '<pre>', print_r( $this->CONFIG_PROPS[$section_name][$setting_name], true ), '</pre>';
                   if (!$this->CONFIG_PROPS[$section_name][$setting_name]['view_web'] ) {
                       unset( $config[ $section_name ] [ $setting_name ] );
                   }
    ...
    /**
     * Convert Args to $_GET
     *
     * When called by command line, convert command line options to $_GET variable
     *
     * Example: php --samples 
     * will look like /?action=samples 
     * 
     * @param none
     * @return void
     */
    public function CliArgs2Get() {

// Script example.php http://php.net/manual/en/function.getopt.php
...
#$shortopts .= "abc"; // These options do not accept values
        ...
        $longopts = array(
            ...
            "config", // No value
            "install", // No value   
            "help-config", // No value   
            
        );

        //the array $options will contain a valid element when an option is passed
    ...
    public function parseQueryVar() {
        ...
        //convert command line options to $_GET query variables
        if ( $this->phpLib()->isCommandLine() ) {
            $this->CliArgs2Get();
        }
    ...
    public function wpSetPermissions() {
        if ( !$this->CONFIG[ 'general' ][ 'set_permissions' ] ) {
            return;
        }
        $this->phpLib()->chmodR(
        ...
        chmod( $this->INSTALLER_DIRECTORY . '/config-site.json', $this->_CONFIG_FILE_PERMISSIONS ); //config-site.json to protect our database password
    ...
    private function _checkConfigFilesExist() {
        ...
        if ( !file_exists( $this->INSTALLER_DIRECTORY . '/config-site.json' ) ) {

            $this->_ERROR_MESSAGES[] = gettext( 'Cannot proceed without a config-site.json file! Create one from `site-config-sample.php`' );
     ...
     *
     * Template Tag - Renders the HTML for the tabs and collapsible accordian 
     *
     * @param none
     * @return void
     */
    public function renderSettingsTabs() {

        ob_start();
        include($this->INSTALLER_DIRECTORY . '/assets/templates/tabs.html');
        //$tabs=file_get_contents($this->INSTALLER_DIRECTORY . '/assets/templates/tabs.html');
        $tabs = ob_get_clean();
        $tags[ '{SITE_SETTINGS_ACCORDIAN}' ] = file_get_contents( $this->INSTALLER_DIRECTORY . '/assets/templates/accordian.htm' );


        $tabs = str_replace( array_keys( $tags ), $tags, $tabs );
     ...
     * Render JS
     *
     * Template Tag - Adds Javascript to Web Form
     *
     * @param none
     * @return void
     */
    public function renderJs() {
        ?>
        <script>var SITE_CONFIG =<?php echo json_encode( $this->getSafeConfig( $this->SITE_CONFIG ) ); ?>;</script>
        <script>var PROFILE_CONFIG = <?php echo json_encode( $this->getSafeConfig( $this->PROFILE_CONFIG ) ); ?>;</script>
        <script>var CONFIG_PROPS = <?php echo json_encode( $this->CONFIG_PROPS ); ?>;</script>         
        <script>var PROFILES = <?php echo json_encode( $this->getProfiles() ); ?>;</script>
   
        <script src="assets/js/mod-wp.js"></script>

        <?php
    }

    /**
     * Lists all settings for help docs
     *
     * Display All Configuration Settings with Description
     *
     * @param none
     * @return void
     */
    public function helpConfig() {
...
//remove json comments
        ...
        $template_html = '<tr class="form-group">'
                . '<td>'
                . '<strong>{ID}</strong>'
                . '<div >'
                . '<em >{DESCRIPTION}</em>'
        ...
        $template_text = ""
                . "{ID}$line_ending"
                . "{DESCRIPTION}$line_ending"
                . "DEFAULT:{VALUE}$line_ending$line_ending";

        $template = ($this->phpLib()->isCommandLine()) ? $template_text : $template_html;

        foreach ( $this->CONFIG_PROPS as $section => $setting ) {

            
            if ( $section==='posts') {
                $tags=array();
                                $tags[ '{DESCRIPTION}' ] = $this->CONFIG_PROPS[ $section ][ 'description' ];
                $tags[ '{LABEL}' ] = $this->CONFIG_PROPS[ $section ][ 'label' ];
                $tags[ '{ID}' ] = $section ;
                                 $tags[ '{VALUE}' ]="See config-profile-sample.json for an example.";

                
        
                $row = str_replace( array_keys( $tags ), $tags, $template );
                $text .= $row;                
                continue;
            }
            
            foreach ( $setting as $setting_name => $setting_props ) {
         
                if ( empty( $this->CONFIG_PROPS[ $section ][ $setting_name ] ) ) {
                    continue;
                }
 $tags=array();
                $tags[ '{DESCRIPTION}' ] = $this->CONFIG_PROPS[ $section ][ $setting_name ][ 'description' ];
                $tags[ '{LABEL}' ] = $this->CONFIG_PROPS[ $section ][ $setting_name ][ 'label' ];
                $tags[ '{ID}' ] = $section . ":" . $setting_name;
                $value=$this->CONFIG_PROPS[ $section ][ $setting_name ][ 'default' ];
                if ( is_bool($value)) {
        ...
        if ( $this->phpLib()->isCommandLine() ) {
            ...
            $page_template = file_get_contents( $this->INSTALLER_DIRECTORY . '/assets/templates/docs.htm' );
            $tags = array();
            $tags[ '{CONTENT}' ] = '<table class="table striped"><th>Setting</th><th>Default</th>'.$text.'</table>';
            $page = str_replace( array_keys( $tags ), $tags, $page_template );
    ...
    /**
     * set Create Config Sample Files
     *
     * Generates the configuration sample file.
     *
     * @param string $type 'profile' or 'site' Whether you want to generate config-site.js or config-profile.js
     * @return void
     */
    public function createConfigSample( $type ) {

        if ( !in_array( $type, array( 'site', 'profile' ) ) ) {
            return;
        }
...
//remove json comments
        ...
        foreach ( $this->CONFIG_PROPS as $section => $setting ) {
            ...
            foreach ( $setting as $setting_name => $setting_props ) {
                ...
                foreach ( $setting_props as $prop_name => $prop_value ) {
                    if ( $prop_name === "default" && $this->CONFIG_PROPS[ $section ][ $setting_name ][ $type ] === true ) {
                        $sample[ $section ][ $setting_name ] = $prop_value;
                    }
                }
            }
        }
        ...
        //  echo nl2br($this->phpLib()->JsonPrettyPrint($sample));

        $sample = $this->phpLib()->JsonPrettyPrint( $this->phpLib()->JsonPrettyPrint( $sample ) );


        $file_path = $this->INSTALLER_DIRECTORY . '/config-' . $type . '-sample.json';

        file_put_contents( $file_path, $sample );

        $line_ending = "\n\n";
        //if called from within the browser, output html
        $heading = "Created sample " . $type . " configuration file " . $file_path;
        if ( !$this->phpLib()->isCommandLine() ) {
